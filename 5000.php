...
// Fetch current balance, account ID, and daily limit
$select_query = "SELECT c.balance, a.user_id, a.daily_limit FROM card c 
...
if ($current_balance >= $withdrawal_amount && $withdrawal_amount <= $daily_limit) {
    ...
    if ($withdrawal_amount > $daily_limit) {
        $message = "Daily limit exceeded. Maximum withdrawal amount per day is $daily_limit";
    } else {
